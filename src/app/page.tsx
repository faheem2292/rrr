...
export default function UswaCollegeFunctional() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      if (parsed.teachers) {
        setTeachers(parsed.teachers.filter((t: any) => t.showOnHomepage).slice(0, 4)); // ✅ Only show teachers marked for homepage
      }
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 font-sans text-slate-900 scroll-smooth">
          ...
          <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-b border-blue-100 shadow-2xl animate-in slide-in-from-top-4 duration-300">
            <div className="flex flex-col p-6 space-y-4">
              <a 
                ...
                className="text-slate-700 hover:text-blue-700 font-bold uppercase tracking-wide py-3 px-4 rounded-xl hover:bg-blue-50 transition-all text-sm"
              >
                🏠 Home
              </a>
              <a 
              ...
              <a 
                href="/login" 
                onClick={handleLinkClick}
                className="bg-linear-to-r from-blue-700 to-blue-900 text-white px-6 py-4 rounded-2xl text-sm font-black uppercase shadow-lg text-center hover:scale-105 transition-all"
              >
                🔐 Admin Login
              </a>
            </div>
      ...
      {/* ═══════════════════════════════════════════════════════════ */}
      {/* STAFF SECTION - MOBILE RESPONSIVE */}
      {/* ═══════════════════════════════════════════════════════════ */}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {displayAnnouncements.length === 0 && (
            <div className="text-center py-16 md:py-20">
              ...
              <p className="text-white/60 font-bold text-base md:text-lg">No announcements at the moment</p>
              <p className="text-white/40 text-xs md:text-sm mt-2">Check back later for updates</p>
            </div>
          )}
        </div>
      </section>
